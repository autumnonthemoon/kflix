...
export default function Search(props: SearchPropsTypes): JSX.Element {
    ...
    useEffect(() => {
        async function fetchAllContent() {
            try {
                const [koreanSeries, koreanMovies, koreanComedy, koreanHorror] = await Promise.all([
                    axios.get(requests.fetchKoreanDramas),
                    axios.get(requests.fetchKoreanMovies),
                    axios.get(requests.fetchComedyMovies),
                    axios.get(requests.fetchHorrorMovies)
                ])

                const koreanSeriesRes = koreanSeries.data.results
                const koreanMoviesRes = koreanMovies.data.results
                const koreanComedyRes = koreanComedy.data.results
                const koreanHorrorRes = koreanHorror.data.results

                setAllContent([...koreanSeriesRes, ...koreanMoviesRes, ...koreanComedyRes, ...koreanHorrorRes])
    ...
    useEffect(() => {
        const results = allContent.filter((item) =>
            (item.title || item.name).toLocaleLowerCase().includes(searchTerm.toLocaleLowerCase())
        )
    ...
    return <main className="search-container">
        <input type="text" className="search-bar" placeholder="Search something.." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
